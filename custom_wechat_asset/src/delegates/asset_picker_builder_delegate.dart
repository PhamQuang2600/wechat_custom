...
import 'package:chat_app/core/constants/theme/app_dimens.dart';
import 'package:chat_app/core/helper/logger.dart';
import 'package:chat_app/core/image/images.dart';
...
abstract class AssetPickerBuilderDelegate<Asset, Path> {
  ...
  bool get effectiveShouldRevertGrid => shouldRevertGrid ?? false;
  ...
  Widget iOSPermissionOverlay(BuildContext context) {
    final Widget _closeButton = Container(
      ...
      padding: EdgeInsets.only(bottom: 12, left: 15),
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [
            Color(0xFF0086DA),
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            ...
            textAlign: TextAlign.center,
            semanticsLabel: 'Không thể truy cập tất cả ảnh',
          ),
          SizedBox(height: AppDimens.height / 30),
          Text(
            'Bạn đã cài đặt chỉ cho phép truy cập một số ảnh nhất định.\n'
            'Đề xuất cho phép truy cập tất cả ảnh',
    ...
    final Widget _goToSettingsButton = InkWell(
      child: Container(
        decoration: BoxDecoration(
    ...
    final Widget _accessLimitedButton = InkWell(
      ...
      child: Container(
        ...
        child: Center(
          child: Text(
            'Tiếp tục với ảnh được chọn',
            style:
                TextStyle(color: Colors.white, fontWeight: FontWeight.w500),
            semanticsLabel: 'Tiếp tục với ảnh được chọn',
          ),
        ),
      ),
    );
...
class DefaultAssetPickerBuilderDelegate
  ...
  FutureOr Function()? onTapSendButton;
  ...
  Future<void> selectAsset(
    ...
    // else if(onTapSendButton!=null) {
    //   await onTapSendButton!();
    //   Navigator.of(context).maybePop(provider.selectedAssets);
    // }
    logger.log('${!multi} && ${provider.selectedAssets.length <= 1}');
    if (!multi && provider.selectedAssets.length <= 1) {
      Navigator.of(context).maybePop(provider.selectedAssets);
    }
  }
  ...
  AssetPickerAppBar appBar(BuildContext context) {
    return AssetPickerAppBar(
      ...
      // : null,
      // actions:  sendButton==null?<Widget>[confirmButton(context)]:null,
          ...
          builder: (_, DefaultAssetPickerProvider p, __) {
            ...
            return Semantics(
              ...
              child: GestureDetector(
                ...
                onTap: () =>
                    selectAsset(context, asset, isSelected, multi: false),
                onLongPress:
  ...
  int assetsGridItemCount({
    ...
    if(currentPathEntity?.name != 'Recent'){
      placeholderCount = 1;
    }
              ...
              children: [
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    height: 20,
                    child: Center(
                      child: Text('${p.selectedAssets.length}'),
                    ),
                  ),
                ),
              ],
  ...
  Widget selectIndicator(BuildContext context, int index, AssetEntity asset) {
    final double indicatorSize = context.mediaQuery.size.width / gridCount / 4;
    ...
    return Selector<DefaultAssetPickerProvider, String>(
      ...
      builder: (BuildContext context, String descriptions, __) {
        ...
        var p = context.read<DefaultAssetPickerProvider>();
        final int selectedIndex = p.selectedAssets.indexOf(asset) + 1;
        ...
        final Widget selectorWidget = GestureDetector(
          ...
          onTap: () => selectAsset(context, asset, selected, multi: true),
          child: Container(
