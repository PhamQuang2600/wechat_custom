...
import 'package:chat_app/core/custom_package/custom_wechat_asset/src/internal/singleton.dart';
import 'package:chat_app/core/custom_package/custom_wechat_asset/src/widget/scale_text.dart';
...
class _ImagePageBuilderState extends State<ImagePageBuilder> {
  ...
  Widget _imageBuilder(BuildContext context, AssetEntity asset) {
    return FutureBuilder(future: asset.file, builder: (context, snapshot) {
      if(snapshot.hasData){
        return RepaintBoundary(child: PhotoView(
          imageProvider: Image.file(snapshot.data!, fit: BoxFit.contain,).image));
      } else{
